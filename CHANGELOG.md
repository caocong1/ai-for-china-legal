...
- `criminal-compliance`：案卷输入脱敏停机；定罪量刑须本次会话 `law-database` / 官方网站核验，不得只靠待验证标注。脱敏与强制库核接到全部刑事编排入口与实践配置共享护栏。
- 施工合同审查、商事合同起草加载合同质量门禁；诉讼代理词/答辩状研究闸门补充十库路由与推理调度。
- 劳动、数据合规、监管实践配置引用十库路由（规章与政策不得混称、禁止推断现行有效）。
